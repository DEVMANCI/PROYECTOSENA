<?php
/**
 * Script para mostrar los detalles de los mujer
...
if ($id == '' || $token == '') {
    ...
    if ($token == $token_tmp) {
        $sql = $con->prepare("SELECT count(id) FROM mujer WHERE id = ? AND activo = 1");
        $sql->execute([$id]);
        if ($sql->fetchColumn() > 0) {
            $sql = $con->prepare("SELECT id, nombre, descripcion, precio, descuento FROM mujer WHERE id = ? AND activo = 1 LIMIT 1");
            $sql->execute([$id]);

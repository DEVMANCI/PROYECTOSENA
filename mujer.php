...
<body class="d-flex flex-column h-100">
                ...
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-4">
                    ...
                    <div class="col mb-2">
                        <div class="card shadow-sm h-100">
                            <?php
                                $id = $row['id'];
                                $imagen = "images/mujer/" . $id . "/principal.jpg";
                                if (!file_exists($imagen)) {
                                    $imagen = "images/no-photo.jpg";
                                }
                                ?>
                            <a
                                href="detailsmujer.php?id=<?php echo $row['id']; ?>&token=<?php echo hash_hmac('sha1', $row['id'], KEY_TOKEN); ?>"><img
                            ...
                            <div class="card-footer bg-transparent">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div class="btn-group">
                                        <a href="detailsmujer.php?id=<?php echo $row['id']; ?>&token=<?php echo hash_hmac('sha1', $row['id'], KEY_TOKEN); ?>"
                                            class="btn btn-sm btn-outline-primary">Detalles</a>
                                    </div>
                                    <button class="btn btn-sm btn-secondary btn-outline" type="button"
                                        onclick="addMujer(<?php echo $row['id']; ?>, '<?php echo hash_hmac('sha1', $row['id'], KEY_TOKEN); ?>')">Agregar</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
    function addMujer(id, token) {
        let url = 'clases/carritomujer.php'

...
$mujer = isset($_SESSION['carrito']['mujer']) ? $_SESSION['carrito']['mujer'] : null;

$lista_carrito = array();

if ($mujer != null) {
    foreach ($mujer as $clave => $cantidad) {
        $sql = $con->prepare("SELECT id, nombre, precio, descuento, $cantidad AS cantidad FROM mujer WHERE id = ? AND activo = 1");
        $sql->execute([$clave]);
